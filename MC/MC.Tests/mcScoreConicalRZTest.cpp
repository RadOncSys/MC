...
namespace MCTests
{
	...
	public:
		...
		TEST_METHOD(ScorePoint)
		{
			...
			Assert::AreEqual(1.0, score->Dose(0, 50), TEST_EPSILON, L"ScorePoint failed", LINE_INFO());
			// 2
			score->ScorePoint(1.0, 0, *(mcRegionReference*)nullptr, mc_particle_t::MCP_PHOTON, geomVector3D(0, 2.0, 5.1));
			Assert::AreEqual(1.0, score->Dose(21, 25), TEST_EPSILON, L"ScorePoint failed", LINE_INFO());
			// 3
			score->ScorePoint(1.0, 0, *(mcRegionReference*)nullptr, mc_particle_t::MCP_PHOTON, geomVector3D(5.0, 0, 20.1));
			Assert::AreEqual(1.0, score->Dose(44, 100), TEST_EPSILON, L"ScorePoint failed", LINE_INFO());
		}
		...
		TEST_METHOD(ScoreLine)
		{
			...
			for (i = 0; i < 175; i++)
				Assert::AreEqual(2.0 / 175, score->Dose(0, i), TEST_EPSILON, L"ScoreLine failed", LINE_INFO());
			...
			Assert::AreEqual(2.0 / 50 + 2.0 / 175, score->Dose(0, 100), TEST_EPSILON * 100, L"ScoreLine failed", LINE_INFO());
			for (i = 1; i < 50; i++)
				Assert::AreEqual(2.0 / 50, score->Dose(i, 100), TEST_EPSILON * 100, L"ScoreLine failed", LINE_INFO());
			...
			Assert::AreEqual(4.0 / 175, score->Dose(49, 174), TEST_EPSILON * 1000, L"ScoreLine failed", LINE_INFO());
			Assert::AreEqual(8.0, sum, TEST_EPSILON * 1000, L"ScoreLine failed", LINE_INFO());
		}
