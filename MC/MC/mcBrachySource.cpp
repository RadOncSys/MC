...
{
	// Спектр Ir-192 представлен количеством фотонов на один распад.
	// Суммарное количество фотонов получается Ю 1, что соответствует физике, 
	// которая говорит, что в результате одного распада может появляться несколько фотонов.
	// Мы здеь не производим перенормировку, что может быть важно в каких-то приложениях.
