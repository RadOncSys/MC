...
double mcEndfEANuclearCrossSectionTable::playmu(double kE, double** pars, int id)
{
	...
	{
		// КАК ДОСТАТЬ AWR и ZA ДЛЯ ПОДСЧЕТА АТОМНОГО И МАССОВОГО НОМЕРА КОМПАУНД ЯДРА И КОНЕЧНОГО ЯДРА
		// КОГДА ЭТО БУДЕТ СДЕЛАНО СДЕЛАТЬ РАСЧЕТ ПАРАМЕТРА "а" И ПОСТРОИТЬ КУМКЛЯТИВНУЮ ГИСТОГРАММУ
	}
}
