...
{
	// Допуск на погрешности геометрических расчетов
	...
	for (int i = 0; i < nlayers_; i++)
	{
		...
		// GG 20200930 - в следующем коде отпала необходимсть, так как теперь это проверяктся внутри getDistanceToRectangleConeOutside
	...
	if (vz < 0)
	{
		...
		// Пересечение с внутренней дыркой не проверяем, так как при правильном коде 
		// мы должны были получить пересечение выше с гранями прямоугольных конусов.
