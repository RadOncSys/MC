...
double mcEndfRecord::GetFloatValue(int idx)
{
	return ParseValue(c[idx], 11);
}

int mcEndfRecord::GetIntValue(int idx)
{
	std::string s(c[idx], 11);
	return std::stoi(s);
}

int mcEndfRecord::GetLineNumber()
{
	std::string s(LineNumber, 5);
	return std::stoi(s);
}

int mcEndfRecord::GetMF()
{
	std::string s(MF, 2);
	return std::stoi(s);
}

int mcEndfRecord::GetMT()
{
	std::string s(MT, 3);
	return std::stoi(s);
}
...
#pragma region mcEndfCrossSectionTable
...
void mcEndfCrossSectionTable::Load(istream& is)
{
	...
	while (!is.fail())
	{
		...
		if (record.LineNumber[3] == ' ' && record.LineNumber[4] == '2')
		{
			...
			if (ninterpolations > 3)
				throw exception((string("Not supported number of interpolations, Z = ") + record.Z[0] + record.Z[1] + "\n" + string("line:") + "\n" + line).c_str());
		}
		else if (record.LineNumber[3] == ' ' && record.LineNumber[4] == '3')
		{
			...
			for (int i = 0; i < ninterpolations; i++)
			{
				...
				interpolationPoints.push_back(npoints);
				interpolationTypes.push_back(atoi(record.c[i*2+1]));
			}
...
double mcEndfCrossSectionTable::get_value(double kE) const
{
	...
	if (i < 0)
		return sigma;
	int interType = interpolationPoints[0] ? interpolationTypes[0] :
		interpolationPoints[1] ? interpolationTypes[1] : interpolationTypes[2];

	if (interType == 5) //log-log
		sigma = exp((log(Values[i + 1]) - log(Values[i])) / (log(Energies[i + 1]) - log(Energies[i])) * (log(kE) - log(Energies[i]))) * Values[i];
	else if (interType == 2) //lin-lin
		sigma = (Values[i + 1] - Values[i]) / (Energies[i + 1] - Energies[i]) * (kE - Energies[i]) + Values[i];
	else if (interType == 3) //lin-log
		sigma = (Values[i + 1] - Values[i]) / (log(Energies[i + 1]) - log(Energies[i])) * (log(kE) - log(Energies[i])) + Values[i];
	else if (interType == 4) //log-lin
		sigma = exp((log(Values[i + 1]) - log(Values[i])) / (Energies[i + 1] - Energies[i]) * (kE - Energies[i])) * Values[i];
	else 
		sigma = (Values[i + 1] - Values[i]) / (Energies[i + 1] - Energies[i]) * (kE - Energies[i]) + Values[i];
}
...
#pragma region mcEndfAngular

mcEndfAngular::mcEndfAngular() : isEmpty(), NE1(0), NE2(0), MT(-1) {}

void mcEndfAngular::Load(std::istream& is, const string& firstLine)
{
	string line, s1, s2, s3, s4;
	mcEndfRecord record;
	int pointCount = 0;
	...
	::memcpy(&record, firstLine.c_str(), 80);
	ZA = record.GetFloatValue(0);
	AWR = record.GetFloatValue(1);
	LTT = record.GetIntValue(3);

	getline(is, line, '\n');
	::memcpy(&record, line.c_str(), 80);
	LI = record.GetIntValue(2);
	LCT = record.GetIntValue(3);

	if (LTT == 3)
	{
		getline(is, line, '\n');
		::memcpy(&record, line.c_str(), 80);
		NE1 = record.GetIntValue(5);
		LValues.resize(NE1);
		getline(is, line, '\n');
		for (int i = 0; i < NE1; i++)
		{
		...
		for (int i = 0; i < NE2; i++)
		{
			...
			int npoints = record.GetIntValue(5);
			getline(is, line, '\n');
...
void mcEndfAngular::dump(std::ostream& os) const
{
	...
	os << "LI = \t" << LI << endl;
	os << "LCT = \t" << LCT << endl;
	os << "NEnergies = \t" << LEnergies.size() << endl;
	os << "NTables = \t" << LValues.size() << endl;
	os << "NTEnergies = \t" << TEnergies.size() << endl;
	os << "NCosinesTables = \t" << Cosines.size() << endl;
	os << "NTValuesTables = \t" << TValues.size() << endl;
	os << endl;

	os << "Energy\tValue" << endl;
	for (int i = 0; i < LEnergies.size(); i++)
	{
		os << LEnergies[i];
...
#pragma region mcEndfEANuclearCrossSectionTable
...
void mcEndfEANuclearCrossSectionTable::Load(std::istream& is, int LAW)
{
			...
			while (!is.fail())
			{
				...
				else
				{
					for (i = 0; i < n_energypoints; i++)
					{
						...
						npoints_out = record.GetIntValue(5);
	...
	else throw exception((string("This LAW type doesn't supported by this code. LAW = ") + to_string(LAW) + "\n" + string("Line: \n") + line).c_str());
}
...
std::string mcEndfProduct::name() const
{
	switch (product_type) {
	case particle_type::neutron: return "neutron";
		break;
	case particle_type::proton: return "proton";
		break;
	case particle_type::deutron: return "deutron";
		break;
	case particle_type::triton: return "triton";
		break;
	case particle_type::alpha: return "alpha";
		break;
	case particle_type::recoils: return "recoils";
		break;
	case particle_type::gammas: return "gammas";
		break;
...
void mcEndfNP::Load(const char* fname, const char* ename)
{
	...
	mcEndfRecord record;
	// Для сохранения объективной информации о данных в файле нужно прочитать заголовок в описательной части
	for (int i = 0; i < 10; i++)
	{
		getline(isEndf, line, '\n');
		if (line.size() < 80)
		...
		int mt = record.GetMT();
		int ln = record.GetLineNumber();

		if (mt == 451 && ln == 1)
		{
			M_ZA = record.GetFloatValue(0);
			M_AWR = record.GetFloatValue(1);
			LRP = record.GetIntValue(2);
		}
		else if (mt == 451 && ln == 3)
		{
			NSUB = record.GetIntValue(4);
		}
	}

	TotalCrossSections.isEmpty = true;
	ElasticCrossSections.isEmpty = true;
	NonElasticCrossSections.isEmpty = true;
	NeutronProductionCrossSections.isEmpty = true;
	GammaProductionCrossSections.isEmpty = true;
	ElasticNeutronAngleDistribution.isEmpty = true;
	GammaAngleDistribution.isEmpty = true;
	...
	while (!isEndf.fail())
	{
		...
		if (string(record.LineNumber, 5) == "99999")
		{
		}

		// Exit. Последняя строка файла. Прерываем не дожидаясь ошибки.
		...
		{
			NonElasticCrossSections.Load(isEndf);
			NonElasticCrossSections.isEmpty = false;
		}

		else if (mf == 3 && mt == 4)
		{
			NeutronProductionCrossSections.Load(isEndf);
			NeutronProductionCrossSections.isEmpty = false;
		}

		// Сечения ядерных реакций 
		// (используется в качестве суиммарных сечений ядерных взаимодействий для протонов
		// и используется в компнаии с MT=2 как полный набор сечений для протонов,
		// т.е. в MC для протонов используются только MT=2 и MT = 5)
		//else if (mf == 3 && mt == 5)
		//{
		...
		{
			GammaProductionCrossSections.Load(isEndf);
			GammaProductionCrossSections.isEmpty = false;
		}

		// Elastic neutron angles
		else if (mf == 4 && mt == 2)
		{
			ElasticNeutronAngleDistribution.Load(isEndf, line);
			ElasticNeutronAngleDistribution.isEmpty = false;
		}

		// Gamma angles
		else if (mf == 4 && mt == 102)
		{
			GammaAngleDistribution.Load(isEndf, line);
			GammaAngleDistribution.isEmpty = false;
		}

		// Энерго-угловые распределения
		
		// MT = 2 - для протонов это упругое рассеяние на ядрах за исключением чисто кулоновского рассеяние,
		// которое в MC моделируется отдельно в виде Мольеровского рассеяния
		// 
		// 
		// 
		
		// TODO: 
		// Упругое рассеяние заряженнных частиц использует LAW = 5, 
		// для которого нет читалки - расписать и убрать комментарии
		...
		// MT = 5 - для протонов это все ядерные реакции
		else if (mf == 6 && mt == 5)
		{
			if (record.LineNumber[3] == ' ' && record.LineNumber[4] == '1')
			{
...
void mcEndfNP::dumpTotalCrossections(ostream& os) const
{
	...
	for (int i = 0; i < Products.size(); i++)
	{
		os << "Product - \t" << Products[i]->name() << "\t #" << i + 1 << endl;
