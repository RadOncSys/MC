...
void mcGaussSource::sample(mcParticle& p, mcThread* thread)
{
	...
	{
		double x =  sigmax_ * mcSamplers::SampleGaussBoxMuller(rng.rnd(), rng.rnd()); // Ğîçûãğûø ìåñòà ğîæäåíèÿ ÷àñòèö â ıëëèïìå íà îñè Õ
		double y =  sigmay_ * mcSamplers::SampleGaussBoxMuller(rng.rnd(), rng.rnd()); // Ğîçûãğûø  ìåñòà ğîæäåíèÿ ÷àñòèö â ıëëèïñå ïî îñè y
		p.p.set(x, y, z_);
		//p.p.set(0, 0, z_);
	}
	p.plast = p.p;
